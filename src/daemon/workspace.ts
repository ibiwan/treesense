...
import { pathToUri, type LspClient } from "./lsp-client.js";
...
export class Workspace {
  ...
  constructor(readonly root: string, profile: LanguageProfile) {
    this.files = new FileRegistry(root);
    this.handles = new HandleTable(this.files);

    // A change we observed and the language server did not must be announced,
    // or its snapshot drifts from ours and every position translated between
    // the two silently stops meaning the same thing. `2` is LSP's `Changed`.
    ...
    this.lsp = profile.createClient(root);
  }

  /**
   * Start the index warming, but do not block on it. READ needs neither the
   * index nor a parse, so the agent can work while the language server is
   * still warming up — only REFS and TRACE wait.
   */
